...
int32 VideoFrame::Clone(VideoFrame* ptr_frame) {
  if (!ptr_frame) {
    LOG(ERROR) << "cannot Clone to a NULL VideoFrame.";
    return kInvalidArg;
  }
  if (buffer_.get() && buffer_length_ > 0) {
    ptr_frame->buffer_.reset(
        new (std::nothrow) uint8[buffer_length_]);  // NOLINT(nothrow)
...
int32 VideoFrameQueue::Init() {
  ...
  for (int i = 0; i < kQueueLength; ++i) {
    VideoFrame* const ptr_frame = new (std::nothrow) VideoFrame;
    if (!ptr_frame) {
...
int32 VideoFrameQueue::Commit(VideoFrame* ptr_frame) {
  ...
  if (MoveFrame(ptr_frame, ptr_pool_frame)) {
    LOG(ERROR) << "VideoFrame Commit MoveFrame failed!";
    return kNoMemory;
  }
...
int32 VideoFrameQueue::Read(VideoFrame* ptr_frame) {
  ...
  if (MoveFrame(ptr_active_frame, ptr_frame)) {
    LOG(ERROR) << "VideoFrame Read MoveFrame failed!";
    return kNoMemory;
  }
...
int32 VideoFrameQueue::MoveFrame(VideoFrame* ptr_source,
