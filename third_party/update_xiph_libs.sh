...
install_ogg_files() {
  ...
  for platform in ${WEBMLIVE_PLATFORMS}; do
    for config in ${WEBMLIVE_CONFIGURATIONS}; do
      webmlive_dirs="${webmlive_dirs} ../libogg/win/${platform}/${config}"
    done
  done

  ogg_dir_array=(${ogg_lib_dirs})
  webmlive_dir_array=(${webmlive_dirs})

  for (( i = 0; i < ${#ogg_dir_array[@]}; ++i )); do
    cp -p "${ogg_dir_array[$i]}/libogg_static.lib" "${webmlive_dir_array[$i]}"
    cp -p "${ogg_dir_array[$i]}/vc120.pdb" "${webmlive_dir_array[$i]}"
  done
}
...
install_vorbis_files() {
  ...
  for platform in ${WEBMLIVE_PLATFORMS}; do
    for config in ${WEBMLIVE_CONFIGURATIONS}; do
      webmlive_dirs="${webmlive_dirs} ../libvorbis/win/${platform}/${config}"
    done
  done
...
# and $MSBUILD_PLATFORMS must match the order in $WEBMLIVE_CONFIGURATIONS and
# $WEBMLIVE_PLATFORMS. The actual term names differ.
readonly MSBUILD_CONFIGURATIONS="Debug Release"
readonly WEBMLIVE_CONFIGURATIONS="debug release"
readonly MSBUILD_PLATFORMS="Win32 x64"
readonly WEBMLIVE_PLATFORMS="x86 x64"
...
# Echo the new text for README.webmlive.
cat << EOF
Ogg and Vorbis libraries updated. README.webmlive information follows:
