...
int VorbisEncoder::Encode(const AudioBuffer& input_buffer) {
  const int bytes_per_sample =
      (input_buffer.config().bits_per_sample + 7) >> 3;
...
int VorbisEncoder::ReadCompressedAudio(AudioBuffer* ptr_buffer) {
  ...
  if (SamplesAvailable()) {
    ...
    ogg_packet packet = {0};
    status = vorbis_analysis(&block_, &packet);
    if (status) {
...
int VorbisEncoder::StorePacket(const ogg_packet* ptr_packet,
...
int64 VorbisEncoder::SamplesToMilliseconds(int64 num_samples) const {
  ...
  const double seconds = sample_rate / sample_count;
