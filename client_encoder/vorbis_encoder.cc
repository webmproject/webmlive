...
// Stores header data from |packet| in |ptr_storage|. Returns |kSuccess| after
// successful allocation of storage and copy of ogg_packet data.
int StoreHeaderPacket(const ogg_packet& packet,
...
// Stores vorbis data from |packet| in |ptr_storage|. Returns |kSuccess| after
// successful allocation of storage and copy of ogg_packet data.
int StoreDataPacket(const ogg_packet& packet,
  ...
  const uint8* ptr_packet = reinterpret_cast<const uint8*>(&packet);
  for (int i = 0; i < sizeof(packet); ++i, ++ptr_packet) {
    ptr_storage->push_back(*ptr_packet);
  }
  const uint8* ptr_packet_payload = packet.packet;
  for (int i = 0; i < packet.bytes; ++i, ++ptr_packet_payload) {
    ptr_storage->push_back(*ptr_packet_payload);
  }
  return VorbisEncoder::kSuccess;
}
...
int VorbisEncoder::Encode(const AudioBuffer& input_buffer) {
  ...
  const int channels = ib.config().channels;
...
int VorbisEncoder::ReadCompressedAudio(AudioBuffer* ptr_buffer) {
  ...
  ogg_packet packet = {0};
  while (SamplesAvailable() && packet.granulepos <= samples_encoded_) {
    ...
    while ((status = vorbis_bitrate_flushpacket(&dsp_state_, &packet)) == 1) {
      if (StoreDataPacket(packet, &ogg_packets_)) {
        LOG(ERROR) << "StoreDataPacket failed: " << status;
        return kCodecError;
      }
    }
  }
  if (ogg_packets_.size() < sizeof(packet)) {
    return kNoSamples;
  }
  if (ogg_packets_.size() < sizeof(packet) + packet.bytes) {
    LOG(ERROR) << "Error reading packets from libvorbis.";
    return kEncoderError;
  }

  // Use timestamp of first packet with non-zero |granualpos| for delay.
  if (audio_delay_ == 0) {
    audio_delay_ = SamplesToMilliseconds(packet.granulepos);
  }
  const int64 timestamp = SamplesToMilliseconds(samples_encoded_);

  // Note: |packet.granulepos| is the last complete sample in the packet.
...
int VorbisEncoder::GenerateHeaders() {
  ...
  status = StoreHeaderPacket(ident_packet,
                             &ident_header_,
  ...
  status = StoreHeaderPacket(comments_packet,
                             &comments_header_,
  ...
  status = StoreHeaderPacket(setup_packet,
                             &setup_header_,
