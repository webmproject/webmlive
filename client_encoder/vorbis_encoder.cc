...
// Stores |ptr_packet|'s data in |ptr_storage|. Returns |kSuccess| after
// successful allocation of storage and copy of ogg_packet data.
int StorePacket(const ogg_packet* ptr_packet,
                boost::scoped_array<uint8>* ptr_storage,
                int32* ptr_length) {
  if (!ptr_packet || !ptr_storage) {
    LOG(ERROR) << "cannot StorePacket with NULL packet/storage.";
    return VorbisEncoder::kInvalidArg;
  }
  ptr_storage->reset(new (std::nothrow) uint8[ptr_packet->bytes]);
  if (!ptr_storage->get()) {
    LOG(ERROR) << "cannot StorePacket, no memory.";
    return VorbisEncoder::kNoMemory;
  }
  memcpy(ptr_storage->get(), ptr_packet->packet, ptr_packet->bytes);
  *ptr_length = ptr_packet->bytes;
...
int VorbisEncoder::Init(const AudioConfig& audio_config,
  ...
  const int kBitsPerIeeeFloat = sizeof(float) * 8;
...
int VorbisEncoder::Encode(const AudioBuffer& input_buffer) {
  const int bytes_per_sample = (input_buffer.config().bits_per_sample + 7) / 8;
  const int num_samples =
      (input_buffer.buffer_length() + (bytes_per_sample - 1)) /
          bytes_per_sample;
  ...
  if (input_buffer.config().format_tag == kAudioFormatPcm) {
    ...
    if (input_buffer.config().channels == 2) {
    ...
    if (input_buffer.config().channels == 2) {
      ...
      memcpy(ptr_encoder_buffer, ieee_float_samples,
