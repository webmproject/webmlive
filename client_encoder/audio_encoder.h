...
class AudioBuffer {
 public:
  ...
  // Notes:
  //   1) When |config.format_tag| is not |kAudioFormatIeeeFloat|, |Init()|
  //      converts the samples to 32 bit/sample IEEE floating point samples.
  //   2) When |config.format_tag| is not |kAudioFormatVorbis|, the samples in
  //      |ptr_data| are deinterleaved before storage.
 ...
 private:
  ...
  // converted samples.
  int ConvertToDeinterleavedIeeeFloat(const AudioConfig& config,
                                      const uint8* ptr_data);
