...
class AudioBuffer {
 public:
  ...
  // |ptr_data| is NULL. Returns |kConversionFailed| when |config.format_tag| is
  // not a |AudioFormat| enumeration value. Returns |kNoMemory| when unable to
  // allocate storage for |ptr_data|.
  // Note: When |config.format_tag| not |kAudioFormatIeeFloat|, |Init()|
  //       converts the samples to 32 bit/sample IEEE floating point samples.
  ...
  int32 Clone(AudioBuffer* ptr_buffer) const;
 ...
 private:
  // Converts audio buffer from |kAudioFormatPcm| to |kAudioFormatIeeFloat|,
  ...
  // converted samples.
  int32 ConvertToIeeeFloat(const AudioConfig& config, const uint8* ptr_data);
