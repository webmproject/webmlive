...
class AudioSinkPin : public CBaseInputPin {
 public:
  typedef WebmEncoderConfig::AudioCaptueConfig AudioConfig;
 ...
 private:
  ...
  // Returns true and sets |actual_config_|, |stride_|, and |audio_format_|
  // when |header| describes a compatible audio format.
  bool StoreAudioFormat(const BITMAPINFOHEADER& header);
...
class AudioSinkFilter : public CBaseFilter {
 public:
  typedef WebmEncoderConfig::VideoCaptureConfig AudioConfig;

  // Stores |ptr_frame_callback|, constructs CBaseFilter and |AudioSinkPin|,
