...
HRESULT AudioSinkPin::CheckMediaType(const CMediaType* ptr_media_type) {
  ...
  // TODO(tomfinegan): Enable support for additional sample sizes. The
  //                   limitation here is in the AudioBuffer class-- it is
  //                   intended mainly to transport audio samples to/from the
  //                   vorbis audio encoder, which supports only floating point
  //                   samples. AudioBuffer only knows how to resample signed
  //                   16-bit PCM audio, so input sample size is limited to
  //                   16-bit PCM samples, and 32 bit IEEE floating point
  //                   samples.
