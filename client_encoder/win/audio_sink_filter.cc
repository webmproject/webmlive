...
HRESULT AudioSinkPin::CheckMediaType(const CMediaType* ptr_media_type) {
  ...
  // TODO(tomfinegan): I want 16 bit integral samples, or 32 bit floating point
  //                   samples; for now I'll just use min/max constants, but
  //                   format bit count constants similar to the ones in
  //                   video_encoder.h should be created.
  ...
  if (actual_config_.format_tag == WAVE_FORMAT_EXTENSIBLE) {
