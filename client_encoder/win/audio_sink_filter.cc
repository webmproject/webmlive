...
HRESULT AudioSinkPin::CheckMediaType(const CMediaType* ptr_media_type) {
  ...
  // TODO(tomfinegan): add proper audio constants (audio_encoder.h, perhaps?).
  ...
  const uint16 bits_per_sample = format.bits_per_sample();
  const uint16 valid_bits_per_sample = format.valid_bits_per_sample();
  if (format.format_tag() == WAVE_FORMAT_EXTENSIBLE &&
      bits_per_sample != valid_bits_per_sample) {
    // TODO(tomfinegan): Not sure how to handle timing w/|bits_per_sample| that
    //                   is not equal to |valid_bits_per_sample|; disabling
    //                   support of |WAVE_FORMAT_EXTENSIBLE| with mismatched
    //                   values until I can get that sorted.
    LOG(INFO) << "rejecting type: sample size and container size mismatch.";
    return VFW_E_TYPE_NOT_ACCEPTED;
  }
...
HRESULT AudioSinkPin::set_config(const AudioConfig& config) {
  ...
  actual_config_ = WebmEncoderConfig::AudioConfig();
  return S_OK;
}
