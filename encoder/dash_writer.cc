...
bool DashWriter::Init(std::string name, std::string id,
  ...
  if (!webm_config.disable_audio) {
    if (!AddAdaptationSet(webm_config, AdaptationSet::kAudio)) {
      LOG(ERROR) << "AddAdaptationSet failed for audio.";
      return false;
    }
  }
  if (!webm_config.disable_video) {
     if (!AddAdaptationSet(webm_config, AdaptationSet::kVideo)) {
      LOG(ERROR) << "AddAdaptationSet failed for video.";
      return false;
    }   
  }
...
bool DashWriter::AddAdaptationSet(const WebmEncoderConfig& webm_config,
