...
int WebmEncoder::PeekVideoTimestamp(int64* timestamp) {
  ...
  if (status == BufferPool<VideoFrame>::kEmpty) {
    ...
    *timestamp = video_encoder_.last_timestamp();
    LOG(INFO) << "NO video frame available ts=" << *timestamp;
  } else {
    *timestamp += timestamp_offset_;
    LOG(INFO) << "video frame available ts=" << *timestamp;
  }
  return status;
}
...
int WebmEncoder::AVEncode() {
  ...
  // Update the video timestamp before actually doing something with it...
  // (if the buffer was empty when we checked we could blindly break the
  // monotonically increasing timestamps rule)
  status = PeekVideoTimestamp(&video_timestamp);
  if (status < 0) {
    LOG(ERROR) << "Video timestamp peek failed: " << status;
    return status;
  ...
  /*while (vorb_enc.time_encoded() > video_timestamp && !video_pool_.IsEmpty()) {
    video_pool_.DropActiveBuffer();
    LOG(INFO) << "Dropped raw video frame (was behind audio).";
    status = PeekVideoTimestamp(&video_timestamp);
    if (status < 0) {
      LOG(ERROR) << "Video timestamp peek failed: " << status;
      return status;
